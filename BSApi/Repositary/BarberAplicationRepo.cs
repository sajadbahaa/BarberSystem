...
namespace Repositary
{
    public class BarberAplicationRepo : Repo<BarberApplications, int>
    {
        ...
        public  async Task<bool> UpdateApplicationWithStatusDraftOrRejectedAsync(BarberApplications entity,bool rejected = false)
        {
            int affect =0;
            var query =  _dbSet.Where(x => x.ApplicationID == entity.ApplicationID
            && x.Status == enApplicationStatus.Draft || x.Status == enApplicationStatus.Rejected);

            if (rejected)
            {
               affect =  await query.ExecuteUpdateAsync
                    (i => i
...
.SetProperty(y => y.UpdateAt, DateOnly.FromDateTime(DateTime.Now))
            ...
            {
                await query .ExecuteUpdateAsync(i => i
                .SetProperty(y => y.CopyFirstName, entity.CopyFirstName)
                .SetProperty(y => y.CopySecondName, entity.CopySecondName)
                .SetProperty(y => y.CopyLastName, entity.CopyLastName)
                .SetProperty(y => y.CopyPhone, entity.CopyPhone)
...
.SetProperty(y => y.UpdateAt, DateOnly.FromDateTime(DateTime.Now))
        ...
        private async Task<bool> _ChangeStatus(int ApplicationID,enApplicationStatus value, enApplicationStatus targetValue, int userID = 0, string ? reason = null)
        {
            int result = 0;
            if (userID == 0)
            {
                result = await _dbSet.Where(x => x.ApplicationID == ApplicationID
...
.SetProperty(y => y.Reason, reason == null ? null : reason)
            ...
            {
                result = await _dbSet.Where(x => x.ApplicationID == ApplicationID
                      && (x.Status == targetValue && x.UserID == userID))
                          .ExecuteUpdateAsync
                              (i => i
          .SetProperty(y => y.Status, value)
          .SetProperty(y => y.Reason, reason == null ? null : reason));
        ...
        public async Task<bool> UpdateApplicationToPendingApprovalAsync(int ApplicationID, int userID)
        {
            return
            await _ChangeStatus(ApplicationID, enApplicationStatus.PendingApproval, enApplicationStatus.Draft, userID); 
        }

        public async Task<bool> UpdateApplicationToCanceledAsync(int ApplicationID,int userID,string ?reason)
        {
            var result = await _dbSet.Where(x => (x.ApplicationID == ApplicationID&&x.UserID==userID)
                      &&
                      (x.Status == enApplicationStatus.Rejected
                      )
        ...
        {
            return
   await _ChangeStatus(ApplicationID, enApplicationStatus.Rejected, enApplicationStatus.PendingApproval,0, reason);
        }
